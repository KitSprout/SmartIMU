...
/* Typedef ---------------------------------------------------------------------------------*/
/* Variables -------------------------------------------------------------------------------*/
extern TIMER_BaseInitTypeDef htick;
extern TIMER_ChannelInitTypeDef htick_cc[3];
...
{
  if (TIMER_EVENTS_COPPARE(htick.Instance, htick_cc[0].Channel) != RESET) {
    TIMER_EVENTS_COPPARE(htick.Instance, htick_cc[0].Channel) = RESET;
    TIMER_TASKS_CLEAR(htick.Instance) = SET;
    htick_cc[0].EventCallback();
  }
}
